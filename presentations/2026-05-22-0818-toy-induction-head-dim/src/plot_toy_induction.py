...
def main() -> None:
    ...
    values = [float(config_rows[name]["top_specialization_mean"]) for name in ORDER]
    plt.bar(x, values, color=COLORS)
    plt.xticks(list(x), LABELS, rotation=15, ha="right")
    plt.ylim(0.0, 1.05)
    plt.ylabel("Top specialization")
    plt.title("Induction task: heterogeneous heads concentrate causal specialization")
    plt.tight_layout()
    ...
    plt.figure(figsize=(8.4, 4.6))
    width = 0.38
    top_delta = [float(config_rows[name]["own_top_loss_delta_mean"]) for name in ORDER]
    random_delta = [float(config_rows[name]["random_same_layer_loss_delta_mean"]) for name in ORDER]
    plt.bar([i - width / 2 for i in x], top_delta, width=width, color=COLORS, label="top head")
    plt.bar([i + width / 2 for i in x], random_delta, width=width, color="#cbd5e1", label="random same layer")
    plt.xticks(list(x), LABELS, rotation=15, ha="right")
    plt.ylabel("Ablation loss delta")
    plt.title("The 64-dim slot is causally load-bearing")
    ...
    plt.figure(figsize=(8.4, 4.6))
    raw_role = [float(config_rows[name]["raw_role_similarity_mean"]) for name in ORDER]
    random_role = [float(config_rows[name]["random_role_similarity_mean"]) for name in ORDER]
    top_match = [float(config_rows[name]["raw_top_head_match_rate"]) for name in ORDER]
    plt.plot(LABELS, raw_role, marker="o", color="#2563eb", label="same slot role")
    plt.plot(LABELS, random_role, marker="o", color="#94a3b8", label="random role")
    plt.plot(LABELS, top_match, marker="o", color="#c2410c", label="top-head match")
    plt.ylim(0.0, 1.05)
    plt.ylabel("Cross-seed stability")
    plt.title("Heterogeneous slots stay stable across seeds")
    ...
    heatmap_configs = ["uniform4", "hetero4", "uniform2", "hetero4_64first"]
    ...
    fig.suptitle("Top causal role follows the 64-dim slot")
