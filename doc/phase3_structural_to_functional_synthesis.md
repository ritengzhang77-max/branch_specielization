...
| SwitchHead parameter-pair diagnostic | In saved two-layer checkpoints, layer-1 output experts were much more similar than layer-1 value experts (`o` cosine `0.8295` vs `v` cosine `0.2884` over seeds 1/2/4/5). | This matches the swap result: layer-1 output swaps are tolerated partly because output projections are similar, while value projections remain distinct. |
