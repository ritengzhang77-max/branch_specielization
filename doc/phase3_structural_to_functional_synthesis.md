...
| SwitchHead parameter-pair diagnostic | In saved one-layer checkpoints, output experts were much more similar than value experts (`o` cosine `0.7887` vs `v` cosine `0.0451`). In saved two-layer checkpoints, layer-1 showed the same pattern (`o` cosine `0.8295` vs `v` cosine `0.2884` over seeds 1/2/4/5). | This matches the swap result: output swaps are tolerated partly because output projections are similar, while value projections remain distinct. |
