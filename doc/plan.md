...
- The first SwitchHead toy bridge showed a sharp distinction between spontaneous
  ...
  persistent functional modularity. A selector-window sweep found a provisional
  threshold: end step 400 was partial, end step 425 split gates but had one
  causal failure, and end step 450 produced reliable 5/5 causal expert
  modularity. This supports the temporal ordering that gate specialization can
  ...
  strength threshold. A strength-duration check confirmed the tradeoff: at end
  step 800, `0.02` was still partial but `0.025` and `0.03` were reliable in 5/5
  seeds, moving the reliable boundary below the 450-step value. A reversed-label
...
- A checkpoint parameter diagnostic supports the swap interpretation: in
  ...
  `0.8295` vs `v` cosine `0.2884` over seeds 1/2/4/5), matching why `swap_o` is
  tolerated while `swap_v` is destructive.
...
- SwitchHead: training code at `github.com/RobertCsordas/moe_attention`; plug-in implementation at `github.com/RobertCsordas/switchhead`. The earlier `github.com/RobertCsordas/moe` target is a related MoE-MLP repo, not the SwitchHead attention code.
